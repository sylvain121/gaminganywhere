...
vencoder_init(void *arg) {
	...
	for(iid = 0; iid < video_source_channels(); iid++) {
		...
		nvStatus = cNvEncoder->CreateEncoder(outputW, outputH);
		nvStatus = cNvEncoder->AllocateIOBuffers(outputW, outputH, false);

		dataPacket = new DataPacket();
		dataPacket->data = new uint8_t[outputW*outputH];
	}
...
vencoder_threadproc(void *arg) {
    ...
    */
	...
	while(vencoder_started != 0 && encoder_running() > 0) {
        ...
        stEncodeFrame.stride[0] = outputW;
        stEncodeFrame.stride[1] = outputW/2;
        stEncodeFrame.stride[2] = outputW/2;
        stEncodeFrame.width = outputW;
        stEncodeFrame.height = outputH;
